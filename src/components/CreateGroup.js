...
const CreateGroup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!formData.groupType || !formData.startPoint || !formData.destination || !formData.startTime || !formData.date || !formData.seats || !formData.uberType) {
        alert('Please fill in all required fields.');
        return;
      }

      const groupsCollection = collection(db, 'groups');
      addDoc(groupsCollection, { ...formData, date: new Date(formData.date), ridees: [user.fullName], seatsAvailable: formData.seats })
          .then(response => {
    ...
    <div className="container mt-5">
      <form onSubmit={handleSubmit} className="create-group-form p-4 shadow rounded">
        ...
        <div className="form-group mb-3">
          ...
          <select name="groupType" className="form-control" id="groupType" value={formData.groupType} onChange={handleChange} required>
        ...
        <div className="form-group mb-3">
          ...
          <input type="time" name="startTime" id="startTime" className="form-control" value={formData.startTime} onChange={handleChange} required />
        </div>

        <div className="form-group mb-3">
          <label htmlFor="date" className="form-label">Date</label>
          <input type="date" name="date" id="date" className="form-control" value={formData.date} onChange={handleChange} required />
        </div>

        <div className="form-group mb-3">
          <label htmlFor="seats" className="form-label">Seats</label>
          <input type="number" name="seats" id="seats" className="form-control" value={formData.seats} onChange={handleChange} required />
        </div>

        <div className="form-group mb-3">
          <label htmlFor="uberType" className="form-label">Uber Type</label>
          <select name="uberType" className="form-control" id="uberType" value={formData.uberType} onChange={handleChange} required>
